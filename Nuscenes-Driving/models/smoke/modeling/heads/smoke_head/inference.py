...
class PostProcessor(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        # we need to change bottom location to center location
...
class KeypointsOnlyPostProcessor(nn.Module):
    def __init__(self,
                 smoker_coder,
                 reg_head,
                 det_threshold,
                 max_detection,
                 pred_2d):
        super(KeypointsOnlyPostProcessor, self).__init__()
        self.smoke_coder = smoker_coder
        self.reg_head = reg_head
        self.det_threshold = det_threshold
        self.max_detection = max_detection
    ...
    def forward(self, predictions, targets):
        ...
        # select top k predicted keypoints
        scores, indexs, clses, ys, xs = select_topk(
            heatmap,
            K=self.max_detection,
        )

        # use the keypoints indexs to select PoI of the regression prediction
        pred_regression = select_point_of_interest(
            batch, indexs, pred_regression
        )
        pred_regression_pois = pred_regression.view(-1, self.reg_head)
        ...
        # divide each target(depth and offsets) from the PoI regression prediction
        pred_depths_offset = pred_regression_pois[:, 0]
        pred_proj_offsets = pred_regression_pois[:, 1:3]
        pred_depths = self.smoke_coder.decode_depth(pred_depths_offset)
        pred_locations = self.smoke_coder.decode_location(
        ...
        # fake location_y(height) dimensions and rotations to get the fake bounding box
        pred_locations[:, 1] = torch.ones_like(pred_depths_offset)  # fix y=1.0
        pred_dimensions = torch.ones_like(pred_regression_pois)  # fix h,w,l = 1.0
        pred_rotys, pred_alphas = torch.zeros_like(pred_depths_offset), torch.zeros_like(pred_depths_offset)  # fix yaw,alpha = 0.0
...
def make_smoke_keypoints_only_post_processor(cfg):
    smoke_coder = SMOKECoder(
    ...
    postprocessor = KeypointsOnlyPostProcessor(
        smoke_coder,
        cfg.MODEL.SMOKE_KEYPOINTS_ONLY_HEAD.REGRESSION_HEADS,
        cfg.TEST.DETECTIONS_THRESHOLD,
        cfg.TEST.DETECTIONS_PER_IMG,
        cfg.TEST.PRED_2D,
    )
