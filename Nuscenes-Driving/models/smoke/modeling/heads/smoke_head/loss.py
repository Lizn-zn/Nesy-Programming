...
from ....modeling.smoke_coder import SMOKECoder
...
class SMOKEKeypointsOnlyLossComputation():
    def __init__(self,
                 smoke_coder,
                 cls_loss,
                 reg_loss,
    ...
    def prepare_predictions(self, targets_variables, pred_regression):
        ...
        # 把计算pre_box3d 的三个变量中的两个换成 target(groundtruth)中的值，分别计算 3 个不一样的 pre_box3d
        # But we only use loss for locs here
        # if self.reg_loss == "DisL1":
        #     pred_box3d_locs = self.smoke_coder.encode_box3d(
        #         targets_variables["rotys"],
        #         targets_variables["dimensions"],
        ...
        #     )
        #
        #     return pred_box3d_locs

    def __call__(self, predictions, targets):
        pred_heatmap, pred_regression = predictions[0], predictions[1]

        # prepare targets
        targets_heatmap, _, targets_variables \
            = self.prepare_targets(targets)
        pred_locations = self.prepare_predictions(targets_variables, pred_regression)
        target_locations = targets_variables['locations']

        target_locations = target_locations.view(
            -1, target_locations.shape[2]
        )
        # targets_regression = targets_regression.view(
        #     -1, targets_regression.shape[2], targets_regression.shape[3]
        # )

        reg_mask = targets_variables["reg_mask"]
        reg_mask = reg_mask.flatten().view(-1,1)
        # reg_mask = reg_mask.view(-1, 1, 1)
        # reg_mask = reg_mask.expand_as(targets_regression)
        reg_mask = reg_mask.expand_as(target_locations)

        # # compute keypoint heatmap loss as Focal Loss
        # hm_loss = self.compute_heatmap_loss(pred_heatmap, targets_heatmap)
        # # compute location regression loss as L1 Loss
        # reg_loss_loc = self.compute_location_loss(pred_locations, target_locations, reg_mask)
        #
        # return hm_loss, reg_loss_loc

        return pred_heatmap, targets_heatmap, pred_locations, target_locations, reg_mask

    def compute_heatmap_loss(self, pred_heatmap, targets_heatmap):
        hm_loss = self.cls_loss(pred_heatmap, targets_heatmap) \
                  * self.loss_weight[0]

        return hm_loss

    def compute_location_loss(self, pred_locations, target_locations, reg_mask):
        reg_loss_loc = F.l1_loss(
            pred_locations * reg_mask,
            target_locations * reg_mask,
            reduction="sum") / (self.loss_weight[1] * self.max_objs)
...
def make_smoke_keypoints_only_loss_evaluator(cfg):
    smoke_coder = SMOKECoder(
        cfg.MODEL.SMOKE_KEYPOINTS_ONLY_HEAD.DEPTH_REFERENCE,
        cfg.MODEL.SMOKE_KEYPOINTS_ONLY_HEAD.DIMENSION_REFERENCE,
        cfg.MODEL.DEVICE,
    )
    focal_loss = FocalLoss(
        cfg.MODEL.SMOKE_KEYPOINTS_ONLY_HEAD.LOSS_ALPHA,
        cfg.MODEL.SMOKE_KEYPOINTS_ONLY_HEAD.LOSS_BETA,
    )

    loss_evaluator = SMOKEKeypointsOnlyLossComputation(
        smoke_coder,
        cls_loss=focal_loss,
        reg_loss=cfg.MODEL.SMOKE_KEYPOINTS_ONLY_HEAD.LOSS_TYPE[1],
        loss_weight=cfg.MODEL.SMOKE_KEYPOINTS_ONLY_HEAD.LOSS_WEIGHT,
        max_objs=cfg.DATASETS.MAX_OBJECTS,
    )
