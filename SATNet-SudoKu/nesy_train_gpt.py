...
def train(net, train_set, test_set, opt):
    ...
    # optim_cls = optimizer.Adam([{'params': net.parameters(), 'lr': adam_lr}], weight_decay=weight_decay)
    ...
    for epoch in range(num_epochs):
        ...
        train_logic_loss = 0
        net.train()
        adjust_learning_rate(optim_cls, epoch, 0.001)
        for batch_idx, sample in enumerate(train_dataloader):
            ...
            inputs = inputs.cuda()
            targets = Z[index, :]
            logits, loss, _ = net(inputs, targets)
            ...
            sys.stdout.write('\r')
            ite = (batch_idx+1)
            sys.stdout.write('| Epoch [%3d/%3d] Iter[%3d/%3d] loss: %.2f logic: %.2f, entr: %.2f Acc: %.3f Rank: %2d t: %.1f/%.1f' \
