...
def eval_traj(logits, per_y, grids, sol_y):
    ...
    # get solving acc
    acc, sol_tp, sol_p, sol_t = get_acc(pred, sol_y)
    # check if collided
    col_cnt, dl_cnt, tot_cnt = check_collide_length(pred, per_y)
    return acc, sol_tp, sol_p, sol_t, col_cnt, dl_cnt, tot_cnt
...
def evaluate_batch_gpt(net, W, b, bmin, bmax, dataloader, threshold=0.0):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            sp, tp = get_point_mat(grids)
            sp = sp.reshape(-1, size**2)
            tp = tp.reshape(-1, size**2)

            def solve_(idx):
                ind1 = torch.where(sp[idx].reshape(-1) == 1)[0]
                ind2 = torch.where(tp[idx].reshape(-1) == 1)[0]
                _, res = maxsat_solver_gpt(output[idx], W[:, 100:200], \
                            bmin-W[:,0:100]@gt[idx].reshape(size**2,1), \
                            bmax-W[:,0:100]@gt[idx].reshape(size**2,1), ind1, ind2)
                return torch.Tensor(res).reshape(1,-1).cuda()

            if batch_idx == 0:
                u = 3
                show(u, logits[u].unsqueeze(dim=0), gt_grids[u], gt_trajs[u])
            ...
            # logits = F.one_hot(logits.reshape(num, 2, 10, 10), num_classes=2)
            # acc, sol_tp, sol_p, sol_t, col_cnt, dl_cnt, tot_cnt = eval_traj(logits, gt, grids, y)
            # print(sol_tp/sol_p, sol_tp/sol_t, col_cnt/tot_cnt, dl_cnt/(tot_cnt-col_cnt))
            
            res = Parallel(n_jobs=16)(delayed(solve_)(idx) for idx in range(num))
            paths = torch.cat(res, dim=0)[:,0:100]
            print((paths != output).sum())
            logits = torch.cat([paths, tmp_y], dim=-1).long()
            logits = F.one_hot(logits.reshape(num, 2, 10, 10), num_classes=2)
            acc, sol_tp, sol_p, sol_t, col_cnt, dl_cnt, tot_cnt = eval_traj(logits, gt, grids, y)
            print(sol_tp/sol_p, sol_tp/sol_t, col_cnt/tot_cnt, dl_cnt/(tot_cnt-col_cnt))
            ...
            total_dl_cnt += dl_cnt
            total += tot_cnt
    print('final result | acc: %.3f precision: %.3f recall %.3f coll rate %.3f dist error %.3f' \
                           %(total_acc/total, total_sol_tp/total_sol_p, total_sol_tp/total_sol_t, total_col_cnt/total, total_dl_cnt/(total-total_col_cnt)))
