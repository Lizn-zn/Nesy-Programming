...
parser.add_argument('--num_epochs', default=600, type=float, help='the number of epochs')
...
parser.add_argument('--clauses', default=2000, type=int, help='the number of clauses')
...
parser.add_argument('--a', default=0.75, type=float, help='alpha in focal loss')
parser.add_argument('--b', default=2.0, type=float, help='gamma in focal loss')
...
# network
net = build_model_resnet(cfg, device, num_layers=18, pretrained=False)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, sample in enumerate(train_dataloader):
        ...
        # inference
        preds = net(inputs).reshape(-1, size**2)
        ...
        sys.stdout.write('\r')
        ite = (batch_idx+1)
        sys.stdout.write('| Epoch [%3d/%3d] Iter[%3d/%3d] loss: %.2f logic: %.2f, entr: %.2f Prec/Recall/Acc: %.3f/%.3f/%.3f Rank: %2d t: %.1f/%.1f' \
                %(epoch, num_epochs, batch_idx+1, len(train_dataloader), train_nn_loss/ite, train_logic_loss/ite, entropy, prec, recall, acc, rank, t1, t2))
        sys.stdout.flush()
