...
parser.add_argument('--derive', default=1, type=int, help='Cuda device.')
...
def bounding_box(net, phi, dataset):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(dataloader):
            ...
            # inference
            inputs = inputs.cuda()
            _, preds = torch.max(net(inputs)[0], dim=-1)
...
if __name__ == "__main__":
    ...
    # res = evaluate(net, W, b, bmin, bmax, test_set, conf_threshold)
    res = evaluate_batch_gpt(net, W, b, bmin, bmax, test_set, conf_threshold)