...
# loading
ckpt = './checkpoint/' + opt.exp_name + '__{}.0_0.t7'.format(opt.len)
# init chektpoint
static_dict = torch.load(ckpt)
phi = static_dict['logic']
W1, b1 = bounding_box(phi, X_train, 1-y_train)
print(W1.shape)
# loading
ckpt = './checkpoint/' + opt.exp_name + '__2.0_0.t7'
# init chektpoint
static_dict = torch.load(ckpt)
phi = static_dict['logic']
W2, b2 = bounding_box(phi, X_train, 1-y_train)
print(W2.shape)

Wtmp = torch.cat([W1, W2], dim=0)
btmp = b1 + b2
print(Wtmp, btmp)
